...
LOG_MODULE_REGISTER(ubx_socket);
...
// Initialise.
static int32_t init() {
  volatile int32_t errorCode = (int32_t)U_ERROR_COMMON_SUCCESS;
  ...
  if (errorCode == 0) {
    ...
    if (!gInitialised) {
      ...
      if ((errnoLocalCell == U_SOCK_ENONE) ||
          (errnoLocalCell == -U_SOCK_ENOSYS)) {
      ...
      if (errnoLocal == U_SOCK_ENONE) {
        ...
        for (size_t x = 0;
             x < sizeof(gStaticContainers) / sizeof(gStaticContainers[0]);
             x++) {
...
static uSockContainer_t *
pContainerFindByDescriptor(uSockDescriptor_t descriptor) {
...
static void closedCallback(uDeviceHandle_t devHandle, int32_t sockHandle) {
  ...
  if (pContainer != NULL) {
    ...
    if (pContainer->socket.pClosedCallback != NULL) {
      pContainer->socket.pClosedCallback(
          pContainer->socket.pClosedCallbackParameter);
      pContainer->socket.pClosedCallback = NULL;
    }
...
static void dataCallback(uDeviceHandle_t devHandle, int32_t sockHandle) {
  ...
  if (pContainer != NULL) {
    ...
    if (pContainer->socket.pDataCallback != NULL) {
      pContainer->socket.pDataCallback(
          pContainer->socket.pDataCallbackParameter);
    }
    U_PORT_MUTEX_UNLOCK(gMutexCallbacks);
  }
}
...
static bool ipv6StringToAddress(const char *pAddressString,
  ...
  for (size_t x = 0; (x < sizeof(digits) / sizeof(digits[0])) && success; x++) {
    ...
    digits[x] = (uint16_t)y;
    success = (pTmp > pAddressString) && (y >= 0) &&
              (y <= (int32_t)USHRT_MAX) &&
  ...
  if (success && (z == sizeof(digits) / sizeof(digits[0]))) {
    ...
    pAddress->ipAddress.address.ipv6[3] =
        (((uint32_t)digits[0]) << 16) | (digits[1]);
    pAddress->ipAddress.address.ipv6[2] =
        (((uint32_t)digits[2]) << 16) | (digits[3]);
    pAddress->ipAddress.address.ipv6[1] =
        (((uint32_t)digits[4]) << 16) | (digits[5]);
    pAddress->ipAddress.address.ipv6[0] =
        (((uint32_t)digits[6]) << 16) | (digits[7]);
...
static int32_t ipAddressToString(const uSockIpAddress_t *pIpAddress,
  ...
  case U_SOCK_ADDRESS_TYPE_V4:
    stringLengthOrError =
        snprintf(pBuffer, sizeBytes, "%u.%u.%u.%u",
  ...
  case U_SOCK_ADDRESS_TYPE_V6:
    ...
    for (int32_t x = 3; (x >= 0) && (stringLengthOrError >= 0); x--) {
      thisLength =
          snprintf(pBuffer, sizeBytes, "%x:%x",
...
static int32_t addressToString(const uSockAddress_t *pAddress,
                               bool includePortNumber, char *pBuffer,
                               size_t sizeBytes) {
  ...
  if (includePortNumber) {
    ...
    if (stringLengthOrError >= 0) {
      thisLength =
          ipAddressToString(&(pAddress->ipAddress), pBuffer, sizeBytes);
      if (thisLength >= 0) {
        sizeBytes -= thisLength;
        pBuffer += thisLength;
...
static int32_t receive(const uSockContainer_t *pContainer,
                       uSockAddress_t *pRemoteAddress, void *pData,
  ...
  do {
    if (pContainer->socket.protocol == U_SOCK_PROTOCOL_UDP) {
      ...
      if (devType == (int32_t)U_DEVICE_TYPE_CELL) {
        negErrnoOrSize =
            uCellSockRead(devHandle, sockHandle, pData, dataSizeBytes);
      } else if (devType == (int32_t)U_DEVICE_TYPE_SHORT_RANGE) {
        negErrnoOrSize =
            uWifiSockRead(devHandle, sockHandle, pData, dataSizeBytes);
      }
    }
    if (negErrnoOrSize < 0) {
      ...
      while (descriptorOrError < 0) {
        ...
        if (pContainerFindByDescriptor(descriptor) == NULL) {
          ...
          if (pContainer != NULL) {
            ...
            LOG_INF("U_SOCK: unable to allocate memory for socket.");
            // Exit stage left
            break;
          }
        }
      ...
      if ((descriptorOrError >= 0) && (pContainer != NULL)) {
        ...
        if (errnoLocal == 0) {
          ...
          if (sockHandle >= 0) {
            ...
            LOG_INF("U_SOCK: socket created, descriptor %d, network handle "
                    "0x%08x, socket handle %d.",
            ...
            LOG_WRN("U_SOCK: underlying socket layer could not create socket "
                    "(errno %d).",
                    errnoLocal);
          }
        }
    ...
    if (pRemoteAddress != NULL) {
      ...
      if (pContainer != NULL) {
        ...
        if (pContainer->socket.state == U_SOCK_STATE_CREATED) {
          ...
          LOG_INF("U_SOCK: connecting socket to \"%.*s\"...",
          ...
          if (errorCode == 0) {
            ...
            LOG_INF("U_SOCK: socket with descriptor %d, network handle 0x%08x, "
                    "socket handle %d, is connected to address \"%.*s\".",
                    descriptor, devHandle, sockHandle,
                    addressToString(&pContainer->socket.remoteAddress, true,
            ...
            errnoLocal = -errorCode;
              LOG_WRN(
                "U_SOCK: underlying layer errno %d on address \"%.*s\", "
                "descriptor/network/socket %d/0x%08x/%d.",
                errnoLocal,
                addressToString(pRemoteAddress, true, buffer, sizeof(buffer)),
...
int32_t uSockClose(uSockDescriptor_t descriptor) {
  ...
  if (errnoLocal == U_SOCK_ENONE) {
    ...
    if (pContainer != NULL) {
      ...
      if (errorCode == 0) {
        LOG_INF("U_SOCK: socket with descriptor %d,"
        ...
        errnoLocal = -errorCode;
         LOG_WRN("U_SOCK: underlying socket layer returned"
...
int32_t uSockOptionSet(uSockDescriptor_t descriptor, int32_t level,
                       uint32_t option, const void *pOptionValue,
                       size_t optionValueLength) {
  int32_t errorCode = (int32_t)U_ERROR_COMMON_SUCCESS;
  int32_t errnoLocal;
  uSockContainer_t *pContainer = NULL;
  ...
  LOG_INF("U_SOCK: option set command %d:0x%04x called"
          " on descriptor %d with value ",
        ...
        if ((level == U_SOCK_OPT_LEVEL_SOCK) &&
          ...
          if ((pOptionValue != NULL) &&
            ...
            pContainer->socket.receiveTimeoutMs =
                ...
                (((int64_t)((const struct timeval *)pOptionValue)->tv_sec) *
                 1000);
            LOG_INF("U_SOCK: timeout for socket descriptor"
                    " %d set to %d ms.",
                    descriptor, (int32_t)pContainer->socket.receiveTimeoutMs);
          } else {
            LOG_WRN("U_SOCK: socket option %d:0x%04x"
          ...
          if (errorCode == 0) {
            // All good
            LOG_INF("U_SOCK: socket option %d:0x%04x"
                    " set to value ",
                    option, level);
          } else {
            // Invalid argument
            errnoLocal = -errorCode;
            LOG_WRN("U_SOCK: errno %d when setting"
                    " socket option %d:0x%04x to value ",
                    errnoLocal, option, level);
          }
          printSocketOption(pOptionValue, optionValueLength);
          LOG_INF("by network handle 0x%08x, socket"
                  " handle %d.",
...
int32_t uSockOptionGet(uSockDescriptor_t descriptor, int32_t level,
                       uint32_t option, void *pOptionValue,
                       size_t *pOptionValueLength) {
  int32_t errorCode = (int32_t)U_ERROR_COMMON_SUCCESS;
  int32_t errnoLocal;
  uSockContainer_t *pContainer = NULL;
      ...
      if ((pOptionValue == NULL) || (pOptionValueLength != NULL)) {
        if ((level == U_SOCK_OPT_LEVEL_SOCK) &&
          ...
          if (errorCode == 0) {
            // All good.
            if (pOptionValue != NULL) {
              LOG_INF("U_SOCK: the value of option %d:0x%04x"
                      " for socket descriptor %d is ",
                      option, level, descriptor);
              printSocketOption(pOptionValue, *pOptionValueLength);
              LOG_INF("according to network handle 0x%08x, socket"
                      " handle %d.",
                      devHandle, sockHandle);
            }
          } else {
            // Set errno
            errnoLocal = -errorCode;
            LOG_WRN("U_SOCK: getting the value of option"
                    " %d:0x%04x for socket descriptor %d from"
                    " network handle 0x%08x, socket handle %d,"
                    " returned errno %d.",
                    option, level, descriptor, devHandle, sockHandle,
                    errnoLocal);
          }
        }
      }
...
int32_t uSockSecurity(uSockDescriptor_t descriptor,
  ...
  if (errnoLocal == U_SOCK_ENONE) {
    ...
    if (pContainer != NULL) {
      ...
      devHandle = pContainer->socket.devHandle;
      sockHandle = pContainer->socket.sockHandle;
      pContainer->socket.pSecurityContext =
          pUSecurityTlsAdd(devHandle, pSettings);
      ...
      if (pRemoteAddress != NULL) {
        ...
        if (pContainer->socket.state == U_SOCK_STATE_CONNECTED) {
          ...
          if ((pContainer->socket.state == U_SOCK_STATE_SHUTDOWN_FOR_WRITE) ||
              (pContainer->socket.state ==
               U_SOCK_STATE_SHUTDOWN_FOR_READ_WRITE)) {
            // Socket is shut down
        ...
        if ((pContainer->socket.protocol == U_SOCK_PROTOCOL_UDP) ||
          ...
          if ((pData == NULL) && (dataSizeBytes > 0)) {
            ...
            if ((pData != NULL) && (dataSizeBytes > 0)) {
              ...
              if (devType == (int32_t)U_DEVICE_TYPE_CELL) {
                errorCodeOrSize =
                    uCellSockSendTo(devHandle, sockHandle, pRemoteAddress,
                                    pData, dataSizeBytes);
                ...
                errorCodeOrSize =
                    uWifiSockSendTo(devHandle, sockHandle, pRemoteAddress,
                                    pData, dataSizeBytes);
                if (errorCodeOrSize > 0) {
                  pContainer->socket.bytesSent += errorCodeOrSize;
                }
...
// Send data.
int32_t uSockWrite(uSockDescriptor_t descriptor, const void *pData,
                   size_t dataSizeBytes) {
  int32_t errorCodeOrSize = (int32_t)U_ERROR_COMMON_SUCCESS;
  int32_t errnoLocal;
  uSockContainer_t *pContainer = NULL;
...
// Receive data.
int32_t uSockRead(uSockDescriptor_t descriptor, void *pData,
                  size_t dataSizeBytes) {
  int32_t errorCodeOrSize = (int32_t)U_ERROR_COMMON_SUCCESS;
  int32_t errnoLocal;
  uSockContainer_t *pContainer = NULL;
  ...
  if (errnoLocal == U_SOCK_ENONE) {
    ...
    if (pContainer != NULL) {
      ...
      if (devType == (int32_t)U_DEVICE_TYPE_CELL) {
        ...
        errnoLocal =
            -uWifiSockRegisterCallbackData(devHandle, sockHandle, dataCallback);
      }
  ...
  if (errnoLocal == U_SOCK_ENONE) {
    ...
    if (pContainer != NULL) {
      ...
      if (devType == (int32_t)U_DEVICE_TYPE_CELL) {
        ...
        errnoLocal = -uWifiSockRegisterCallbackClosed(devHandle, sockHandle,
                                                      closedCallback);
      }
  ...
  if (errnoLocal == U_SOCK_ENONE) {
    ...
    if (pLocalAddress != NULL) {
      ...
      if (pContainer != NULL) {
        ...
        if (devType == (int32_t)U_DEVICE_TYPE_CELL) {
          errnoLocal =
              -uCellSockGetLocalAddress(devHandle, sockHandle, pLocalAddress);
        } else if (devType == (int32_t)U_DEVICE_TYPE_SHORT_RANGE) {
          errnoLocal =
              -uWifiSockGetLocalAddress(devHandle, sockHandle, pLocalAddress);
        }
      }
...
int32_t uSockGetHostByName(uDeviceHandle_t devHandle, const char *pHostName,
  ...
  if (errnoLocal == U_SOCK_ENONE) {
    ...
    if ((pHostName != NULL) && (pHostIpAddress != NULL)) {
      ...
      if (devType == (int32_t)U_DEVICE_TYPE_CELL) {
        errnoLocal =
            -uCellSockGetHostByName(devHandle, pHostName, pHostIpAddress);
      } else if (devType == (int32_t)U_DEVICE_TYPE_SHORT_RANGE) {
        errnoLocal =
            -uWifiSockGetHostByName(devHandle, pHostName, pHostIpAddress);
      }
...
// Note: this does not need to reference the underlying
// cell/wifi socket layer.
int32_t uSockAddressToString(const uSockAddress_t *pAddress, char *pBuffer,
                             size_t sizeBytes) {
  int32_t stringLengthOrError = (int32_t)U_ERROR_COMMON_INVALID_PARAMETER;
